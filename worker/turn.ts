...
export async function mintIceServers(env: TurnEnv, mode: IceMode = "direct"): Promise<IceServer[]> {
  ...
  if (!id || !token) return STUN_FALLBACK;
  ...
  try {
    ...
    if (!Array.isArray(servers) || servers.length === 0) return STUN_FALLBACK;
    ...
    const turnOnly = servers.map(normalizeTurnServer).filter((s): s is IceServer => s !== null);
    if (!turnOnly.length) return STUN_FALLBACK;
...
function normalizeTurnServer(server: IceServer): IceServer | null {
  ...
  const chosen = stable.length ? stable : fallback;
